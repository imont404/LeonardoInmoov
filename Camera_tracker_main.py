...
class App(QWidget):
    ...
    def __init__(self):
        ...
        self.MIN_AREA = 4000       # Área mínima para detectar movimiento
        self.BLUR_SIZE = (20, 20) # Tamaño del desenfoque
        self.THRESHOLD_SENSITIVITY = 45 # Sensibilidad del umbral
    ...
    def record(self):  #video recording
        ...
        gray_img_prev = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

        while(self.rec):

            ret, img = self.cap.read() #CAPTURE IMAGE
            gray_img_curr = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

            motion_detected, motion_center = self.motion_detect(gray_img_prev, gray_img_curr)
            if motion_detected:
                print("Movimiento detectado en:", motion_center)
    ...
    def motion_detect(self, gray_img_1, gray_img_2):
        motion_found = False
        biggest_area = self.MIN_AREA
        differenceimage = cv2.absdiff(gray_img_1, gray_img_2)
        differenceimage = cv2.blur(differenceimage, self.BLUR_SIZE)
